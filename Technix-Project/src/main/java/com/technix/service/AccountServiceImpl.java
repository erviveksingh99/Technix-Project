...
public class AccountServiceImpl implements AccountService {
    ...
    public ResponseEntity<Account> getAccountById(int accountId, int companyId) {
        try {
            ...
            List<Ledger> filteredLedgers = account.getLedger().stream()
                    .filter(ledger -> ledger.getCompanyId() == companyId || ledger.getCompanyId() == 0)
                    .collect(Collectors.toList());
            account.setLedger(filteredLedgers);
...
//        }
      ...
      //******************************Method to filter account based on companyId************************************
    //This method to filter the data and send to the accountList
...
//        }
        accountList = accountRepo.findByCompanyIdAndAccountSubId(companyId);
        List<Account> assetAndLiabilitiesAccounts = accountList.stream()
                .filter(account -> account.getAccountSubId() == null &&
                        (account.getAccounts().equalsIgnoreCase("Assets") ||
                                account.getAccounts().equalsIgnoreCase("Liabilities")))
                .collect(Collectors.toList());

       /* // Filter ledgers and child accounts
        assetAndLiabilitiesAccounts.forEach(account -> {
            account.setLedger(account.getLedger().stream()
                    .filter(ledger -> ledger.getCompanyId() == 0 || ledger.getCompanyId() == companyId)
                    .collect(Collectors.toList()));
            account.setChildAccounts(filterChildAccounts(account.getChildAccounts(), companyId));
        ...
        return ResponseEntity.ok(assetAndLiabilitiesAccounts);
    }
